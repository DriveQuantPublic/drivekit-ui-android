...
internal class DKXAxisRenderer(
    ...
    companion object {
        private val backgroundColor = DKColors.secondaryColor
        private val selectedTextColor = if (backgroundColor.shouldInvertTextColor(DKColors.mainFontColor)) {
            Color.WHITE
        } else {
    ...
    override fun drawLabel(
        ...
        if (index != null && index == selectedIndex && text != null && canvas != null) {
            ...
            mAxisLabelPaint.color = DKColors.mainFontColor
        }
...
internal class CustomViewPortHandler : ViewPortHandler() {
    ...
    override fun isInBoundsBottom(y: Float): Boolean {
        val convertedY = (y * 100f).toInt().toFloat() / 100f
        return mContentRect.bottom + GraphConstants.GRAPH_LINE_WIDTH.convertDpToPx() >= convertedY
    }
}
